...
# 6_mad_radiometric_calibration.py has a memory leak caused by reading in data
...
# tgt_dir = 'data/nitze_regions/arosics_output/calibrated_files/'
tgt_files = []
...
# chunk up tgt_files for subprocesses of 10 or less
chunk_size = 5
iterations = math.ceil(len(tgt_files)/chunk_size)
# iterate over chunks, running 6_mad_radiometric_calibration.py each time
for chunk in range(0, iterations):
    print('Starting subprocess ' + str(chunk), flush = True)
    subprocess.run(['python', '6_mad_radiometric_calibration.py'],
                   capture_output = True,
                   timeout = 3600)
    gc.collect()
