...
# Read in data
ref_dir = '/explore/nobackup/people/hrodenhi/rts_mapping/planet_processing_test/data/yg_train_regions/sentinel_data'
base_dir = '/explore/nobackup/people/hrodenhi/rts_mapping/planet_processing_test/data/yg_train_regions/planet_data'
ref_files = []
...
mask_dir = '/explore/nobackup/people/hrodenhi/rts_mapping/planet_processing_test/data/yg_train_regions/arosics_output/masked_files/'
makemydir(mask_dir)
out_dir = '/explore/nobackup/people/hrodenhi/rts_mapping/planet_processing_test/data/yg_train_regions/arosics_output/calibrated_files/'
makemydir(out_dir)
out_fig_dir = '/explore/nobackup/people/hrodenhi/rts_mapping/planet_processing_test/data/yg_train_regions/arosics_output/figures/'
...
for tgt_file, udm_file in zip(tgt_files[start_idx:], udm_files[start_idx:]):
    ...
    region = tgt_file.split('/')[10].split('_')[2]
    masked = os.listdir('/explore/nobackup/people/hrodenhi/rts_mapping/planet_processing_test/data/yg_train_regions/arosics_output/masked_files/')
    masked = [string.split('_masked')[0] for string in masked]

    if region + '_' + tgt_file.split('/')[-1].split('.')[0] not in masked:
    ...
    calibrated = os.listdir('/explore/nobackup/people/hrodenhi/rts_mapping/planet_processing_test/data/yg_train_regions/arosics_output/calibrated_files/')
    calibrated = [string.split('_arosics')[0] for string in calibrated]
    
    if region + '_' + tgt_file.split('/')[-1].split('.')[0] not in calibrated:
        ...
        # /explore/nobackup/people/hrodenhi/rts_mapping/planet_processing_test/data/yg_train_regions/planet_data/region_x/unique_id/PSScene/file.tif
        ref = [file for file in ref_files if re.match('.*' + crs + 'N_' + region + '.tif$', file)][0]
...
masked_files = []
for root, subdirs, files in os.walk('/explore/nobackup/people/hrodenhi/rts_mapping/planet_processing_test/data/yg_train_regions/arosics_output/masked_files/'):
    for file in files:
